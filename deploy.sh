...
if [ "$1" == "--create" ];then
	...
	#docker pull madebian9:latest
	docker pull madebian9
  ...
  id_first=$(docker ps -a --format "{{ .Names }}" |grep "oki-vmparc" | sed s/".*-vmparc"//g  | sort -nr | head -1)
    ...
    #docker run -tid --name ${USERNAME}-vmparc${i} priximmo/stretch-systemd-ssh:v3.2
    #docker run -tid --name ${USERNAME}-vmparc${i} madebian9:latest
    docker run -tid --name ${USERNAME}-vmparc${i} madebian9
